...
excel_path = os.path.join(BASE_DIR, "questions", "interview_questions.xlsx")
...
try:
    ...
    if not os.path.exists(cred_path):
        logger.warning(f"Missing {cred_path}. Proceeding without Firebase.")
    else:
    ...
    logger.error(f"Firebase authentication failed: Invalid JWT Signature. Check firebase-adminsdk.json. Error: {str(e)}")
...
def get_questions():
    if not hasattr(get_questions, 'cache'):
        try:
            ...
            logger.info(f"Loaded {len(get_questions.cache)} questions.")
        except Exception as e:
...
def send_summary_email(user_email, user_name, user_id, summary_data):
    try:
        ...
        msg["From"] = EMAIL_USER
        msg["To"] = user_email
        msg["Subject"] = "Your Excel Interview Summary"

        body = f"""
        Dear {user_name},

        **Important**: Your User ID is: {user_id}
        Please save this User ID to log in for future interviews. Do not share it.

        Thank you for completing the Excel Interview. Below is your summary:

        Average Score: {summary_data['avg_score']:.2f}/{MAX_SCORE}
        Strengths: {summary_data['strengths']}
        Weaknesses: {summary_data['weaknesses']}

        Detailed Feedback:
        """
        for q, f, s in summary_data["detailed_feedback"]:
            body += f"\nQuestion: {q}\nScore: {s}/{MAX_SCORE}\nFeedback: {f}\n"
