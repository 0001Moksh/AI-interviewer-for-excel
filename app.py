...
#     return render_template("guidelines.html",num_questions=num_questions, session_id=session_id)
...
# Validate critical environment variables
if not any(GEMINI_API_KEYS):
    logger.error("No valid GEMINI_API_KEYS provided")
    raise ValueError("Missing GEMINI_API_KEYS")
...
def sanitize_input(text):
    if not isinstance(text, str):
        return ""
    # Remove potentially malicious characters or patterns
    text = re.sub(r'[<>;{}]', '', text)
...
# LangChain-based evaluation function
def evaluate_answer(question, expected, user_answer, history):
    ...
    for msg in history:
        if msg["role"] == "user":
            langchain_history.append(HumanMessage(content=msg["parts"][0]))
        elif msg["role"] == "model":
            langchain_history.append(AIMessage(content=msg["parts"][0]))
    ...
    for api_key in GEMINI_API_KEYS:
        ...
        try:
            ...
            history.append({"role": "user", "parts": [prompt_template.format(
                history=langchain_history,
                question=question,
                expected=expected,
                user_answer=user_answer,
...
def summary(session_id):
    ...
    try:
        ...
        basics_avg = sum(scores[:3]) / min(3, len(scores))
        advanced_avg = sum(scores[6:]) / len(scores[6:]) if len(scores) > 6 else 0
        strengths = "Strong in basics" if basics_avg > 7 else "Needs basics improvement"
        weaknesses = "Improve advanced skills" if advanced_avg < 7 else "Good advanced skills"
        detailed_feedback = list(zip(questions_asked, feedbacks, scores))
